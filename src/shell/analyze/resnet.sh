...
do
    for file in ${files[@]}; 
    do
        simple_hypersearch "python -m layersnotequalinAT.src.visualization.perturbation train_type=retrain retrain_code=$retrain_code $cutoff_or_single={cutoff}" -p cutoff ${cutoffs[@]} | simple_gpu_scheduler --gpus ${gpus[@]}
    done 
done
